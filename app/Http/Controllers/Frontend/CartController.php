...
class CartController extends Controller
{
    ...
    public function increase(CartDetails $cartDetails): RedirectResponse
    {
        $cartDetails->setQuantity($cartDetails->quantity()+1);
        $cartDetails->save();
        return redirect($this->return_url);
    }

    public function decrease(CartDetails $cartDetails): RedirectResponse
    {
        $quantity = $cartDetails->quantity();
        if($quantity > 1) {
        $cartDetails->setQuantity($quantity-1);
        $cartDetails->save();
        }
        return redirect($this->return_url);
    }
